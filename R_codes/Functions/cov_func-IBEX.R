...
nonfrozen_matern_uni_cov_OLD <- function(theta, wind, wind_var, max_time_lag, LOCS, h_mat){
...
matern_multi_step_one <- function(theta, DIST){
	...
	S <- ifelse(DIST != 0, sigma2 * ( sqrt(2 * nu) * DIST / beta)^nu * besselK(sqrt(2 * nu) * DIST / beta, nu) / (2^(nu - 1) * gamma(nu)), sigma2)
	#S <- ifelse(DIST != 0, sigma2 * (DIST / beta)^nu * besselK(DIST / beta, nu) / (2^(nu - 1) * gamma(nu)), sigma2)
...
nonfrozen_matern_cov_multi_advec_small_scale <- function(theta, wind_mu1, wind_mu2, wind_var1, wind_var2, wind_var12, max_time_lag = 0, q = 2, LOCS){
	...
	locs1 <- locs <- cbind(LOCS, rep(1, nrow(LOCS)))

	for(tt in 2:TT){
		locs <- rbind(locs, cbind(LOCS, rep( tt, nrow(LOCS))))
	}
...
nonfrozen_matern_cov_multi_advec_small_scale_NEW <- function(theta, wind_mu1, wind_mu2, wind_var1, wind_var2, wind_var12, max_time_lag = 0, q = 2, LOCS){

	###################												###################
	################### RETURNS a q * nrow(LOCS) * (max_time_lag + 1) x q * nrow(LOCS) * (max_time_lag + 1) matrix 	###################
	###################												###################	

	start_time <- round(theta[7], 0)

	locs1 <- locs <- cbind(LOCS, rep(start_time, nrow(LOCS)))

	for(tt in 1:(TT - 1)){
		locs <- rbind(locs, cbind(LOCS, rep(start_time + tt, nrow(LOCS))))
	}
	...
	for(tt in 0:max_time_lag){
		...
		locs2 <- cbind(LOCS, rep(start_time + tt, nrow(LOCS)))
...
nonfrozen_matern_cov_multi_advec_small_scale_NEWNEW <- function(theta, wind_mu1, wind_mu2, wind_var1, wind_var2, wind_var12, max_time_lag = 0, q = 2, LOCS){

	###################												###################
	################### RETURNS a q * nrow(LOCS) * (max_time_lag + 1) x q * nrow(LOCS) * (max_time_lag + 1) matrix 	###################
	###################												###################	

	locs1 <- locs <- cbind(LOCS, rep(0, n))
	...
	for(tt in 0:max_time_lag){
		...
		#Sigma_temp1 <- nonfrozen_rcpp_NEW(Loc1 = matrix(locs1[1, ], nrow = 1), Loc2 = matrix(locs2[1, ], nrow = 1), param = c(sigma2[1], beta, nu[1]), v_mean1 = wind_mu1, v_mean2 = wind_mu2, v_var1 = wind_var1, v_var2 = wind_var2, v_var12 = wind_var12)
		Sigma_temp1 <- nonfrozen_rcpp_NEW(Loc1 = locs1, Loc2 = locs2, param = c(sigma2[1], beta, nu[1]), v_mean1 = wind_mu1, v_mean2 = wind_mu2, v_var1 = wind_var1, v_var2 = wind_var2, v_var12 = wind_var12)
		Sigma_temp2 <- nonfrozen_rcpp_NEW(Loc1 = locs1, Loc2 = locs2, param = c(sigma2[2], beta, nu[2]), v_mean1 = wind_mu2, v_mean2 = wind_mu1, v_var1 = wind_var2, v_var2 = wind_var1, v_var12 = t(wind_var12))
...
nonfrozen_matern_cov_multi_advec <- function(theta, wind_mu1, wind_mu2, wind_var1, wind_var2, wind_var12, max_time_lag = 0, q = 2, LOCS){
	...
	output <- foreach(tt = 1:TT, .packages = "MASS", .noexport = "nonfrozen_rcpp") %dopar% {
		...
		#Sigma_temp <- nonfrozen_rcpp_NEW(Loc1 = locs1, Loc2 = locs2, param = c(sigma2[1], beta, nu[1]), v_mean1 = wind_mu1, v_mean2 = wind_mu2, v_var1 = wind_var1, v_var2 = wind_var2, v_var12 = wind_var12)
	...
	output <- foreach(tt = 1:TT, .packages = "MASS", .noexport = "nonfrozen_rcpp") %dopar% {
		...
		#Sigma_temp <- nonfrozen_rcpp_NEW(Loc1 = locs1, Loc2 = locs2, param = c(sigma2[2], beta, nu[2]), v_mean1 = wind_mu2, v_mean2 = wind_mu1, v_var1 = wind_var2, v_var2 = wind_var1, v_var12 = t(wind_var12))
...
nonfrozen_lmc_cov <- function(theta, wind_mu1, wind_mu2, wind_var1, wind_var2, max_time_lag, q = 2, LOCS){
	...
	S[[2]] <- Sigma22
  	...
  	S1 <- rbind(cbind(alpha[1,1]^2 * S[[1]] + alpha[1,2]^2 * S[[2]], alpha[1,1] * alpha[2,1] * S[[1]] + alpha[1,2] * alpha[2,2] * S[[2]]), cbind(alpha[1,1] * alpha[2,1] * S[[1]] + alpha[1,2] * alpha[2,2] * S[[2]], alpha[2,1]^2 * S[[1]] + alpha[2,2]^2 * S[[2]]))
  	#S1 <- rbind(cbind(0.6 * S[[1]] + 0.4 * S[[2]], 0.2 * S[[1]] + 0.3 * S[[2]]), cbind(0.2 * S[[1]] + 0.3 * S[[2]], 0.1 * S[[1]] + 0.9 * S[[2]]))
  
  	return(S1)
}

nonfrozen_lmc_cov_R3 <- function(theta, wind_mu1, wind_mu2, wind_mu3, wind_mu4, wind_var1, wind_var2, wind_var3, wind_var4, max_time_lag, q = 2, LOCS){
	...
	clusterExport(cl, c("root", "LOCS", "TT", "n", "nu", "beta", "var", "wind_mu1", "wind_mu2", "wind_mu3", "wind_mu4", "wind_var1", "wind_var2", "wind_var3", "wind_var4", "locs1"), envir = environment())
	clusterEvalQ(cl, source(file = paste(root, "R_codes/Functions/load_packages.R", sep = '')))
	clusterEvalQ(cl, sourceCpp(file = paste(root, "R_codes/Functions/spatially_varying_parameters2.cpp", sep = '')))
	clusterEvalQ(cl, source(file = paste(root, "R_codes/Functions/auxiliary_functions.R", sep = '')))
	...
	output <- foreach(tt=0:(TT - 1), .packages = "MASS", .noexport = "nonfrozen_rcpp") %dopar% {
		...
		Sigma_temp <- nonfrozen_rcpp(Loc1 = locs1, Loc2 = locs2, param = c(var[2], beta[2], nu[2]), v_mean = wind_mu3, v_var = wind_var3)

		return(Sigma_temp)

	}

	Sigma33 <- toeplitz_mat(output)

	output <- foreach(tt=0:(TT - 1), .packages = "MASS", .noexport = "nonfrozen_rcpp") %dopar% {

		locs2 <- cbind(LOCS, rep(tt, n))

		Sigma_temp <- nonfrozen_rcpp(Loc1 = locs1, Loc2 = locs2, param = c(var[2], beta[2], nu[2]), v_mean = wind_mu4, v_var = wind_var4)

		return(Sigma_temp)

	}

	Sigma44 <- toeplitz_mat(output)
	...
	S[[4]] <- Sigma44
  	...
  	S1 <- rbind(cbind(0.4 * S[[1]] + 0.3 * S[[2]] + 0.2 * S[[3]] + 0.1 * S[[4]], 0.2 * S[[1]] + 0.15 * S[[2]] + 0.1 * S[[3]] + 0.05 * S[[4]]),
              	cbind(0.2 * S[[1]] + 0.15 * S[[2]] + 0.1 * S[[3]] + 0.05 * S[[4]], 0.1 * S[[1]] + 0.2 * S[[2]] + 0.3 * S[[3]] + 0.4 * S[[4]]))
  
  	return(S1)
}

frozen_matern_cov_multi_advec_bhatia <- function(theta, wind1, wind2, max_time_lag = 0, q = 2, LOCS){
	...
	locs1 <- locs2 <- locs12 <- locs21 <- coords <- LOCS
  
  	if (max_time_lag > 0){
		for (tt in 1:max_time_lag){
			temp_locs <- cbind(coords[, 1] - tt * w[1], coords[, 2] - tt * w[2])
			locs1 <- rbind(locs1, temp_locs)
		}
  	} 
	...
	w <- wind12
  	...
  	if (max_time_lag > 0){
		for (tt in 1:max_time_lag){
			temp_locs <- cbind(coords[, 1] - tt * w[1], coords[, 2] - tt * w[2])
			locs12 <- rbind(locs12, temp_locs)
		}
  	} 

	dist1 <- distR_C(locs1, locs1)
	dist2 <- distR_C(locs2, locs2)
	dist12 <- distR_C(locs12, locs12)
	...
	S <- matrix(NA,  q * nrow(dist1), q * nrow(dist1))
  
	for(i in 1:q){
		for(j in 1:i){

			temp <- (i - 1) * nrow(dist1) + 1:nrow(dist1)
	      		temp1 <- (j - 1) * nrow(dist1) + 1:nrow(dist1)
	      
	      		if(i == j){
		
				dist0 <- DIST[[i]]
				temp2 <- ifelse(dist0 != 0, sigma2[i] * (dist0 / beta)^nu[i] * besselK(dist0 / beta, nu[i]) / (2^(nu[i] - 1) * gamma(nu[i])), sigma2[i])
				#temp2 <- ifelse(dist0 != 0, sigma2[i] * exp(-dist0^2), sigma2[i])
				S[temp, temp1] <- temp2
				...
				temp3 <- (dist0 / beta)^nu3 * besselK(dist0 / beta, nu3)/(2^(nu3 - 1) * gamma(nu3)) * sqrt(sigma2[i] * sigma2[j]) * rho
				#temp3 <-  exp(-dist0^2) * sqrt(sigma2[i] * sigma2[j]) * rho
				temp3[is.na(temp3)] <- sqrt(sigma2[i] * sigma2[j]) * rho
				S[temp, temp1] <- temp3
				S[temp1, temp] <- t(temp3)
		      }
	...
	C11 <- S[1:(n * (max_time_lag + 1)), 1:(n * (max_time_lag + 1))]
	C22 <- S[n * (max_time_lag + 1) + 1:(max_time_lag + 1), n * (max_time_lag + 1) + 1:(n * (max_time_lag + 1))]
	C12 <- S[1:(n * (max_time_lag + 1)), n * (max_time_lag + 1) + 1:(n * (max_time_lag + 1))]

	new_C12 <- sqrtm(C11) %*% (C12 * 0.5) %*% sqrtm(C22)

	SS <- rbind(cbind(C11, new_C12), cbind(t(new_C12), C22))

	return(SS)
}

frozen_matern_cov_multi_advec <- function(theta, wind1, wind2, max_time_lag = 0, q = 2, LOCS){
	...
	dist1 <- distR_C(locs1, locs1)
	dist2 <- distR_C(locs2, locs2)
	dist12 <- distR_C(locs1, locs2)
	dist21 <- distR_C(locs2, locs1)
	...
	for(i in 1:q){
		for(j in 1:q){
			...
			temp <- (i - 1) * nrow(dist1) + 1:nrow(dist1)
				...
				if(j > i)	dist0 <- DIST[[3]]	else	dist0 <- DIST[[4]]
				nu1 <- nu[i]
				nu2 <- nu[j]
				nu3 <- (nu1 + nu2)/2
...
lmc_cov <- function(theta, wind1, wind2, max_time_lag, q = 2, LOCS){
	...
	alpha <- matrix(c(theta[7], theta[8], theta[9], theta[10]), ncol=2, byrow=T)
  	...
  	for(i in 1:q){
		...
		if (max_time_lag > 0){
    		...
    		SS <- ifelse(dist0 != 0, var[i] * (dist0 / beta[i])^nu[i] * besselK(dist0 / beta[i], nu[i]) / (2^(nu[i] - 1) * gamma(nu[i])), var[i])
